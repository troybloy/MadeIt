  ...
  useEffect(() => {
    ...
    if (!user) {
      ...
      if (
        ...
        errors.push(
          'Item name must be between 2 and 50 characters and must not contain any doubled or more white space'
        )
      }
      if (
        parsedPrice >= 100000.0 ||
        parsedPrice <= 0.0 ||
        !onlyNums.test(item_price) ||
        item_price.includes('  ')
      ) {
        errors.push(
          "Item prices must must not contain any white space and be a number greater than '$0.00' and less than '$100,000.00' with no commas (prices with cents must be in '$.$$' format)"
        )
      }
      if (
        item_description.length < 2 ||
        item_description.length > 255 ||
        item_description.includes('  ')
      ) {
        errors.push(
          'Item description must be between 2 and 255 characters and contain no doubled white space'
        )
      }
    ...
    <div className='form-outer-container'>
      <form onSubmit={handleSubmit}>
        ...
        <div className='required-field'>
          (Fields labeled with&nbsp;<div className='asterisk'>*</div>&nbsp;are
          required)
        </div>
        <div className='required-field'>
          (Fields must not contain any doubled or more white space)
        </div>
        <div className='form-container'>
          ...
          <div>
            <label htmlFor='Item Name' className='form-field-label'>
              Item Name&nbsp;<div className='asterisk'>*</div>
            </label>
            <input
          ...
          <div>
            <label htmlFor='Item Price' className='form-field-label'>
              Item Price&nbsp;<div className='asterisk'>*</div>
            </label>
            <input
              className='form-field'
              name='Item Price'
          ...
          <div>
            <label htmlFor='Item Description' className='form-field-label'>
              Item Description&nbsp;<div className='asterisk'>*</div>
            </label>
            <input
              className='form-field'
          ...
          <div>
            <label htmlFor='Item Image' className='form-field-label'>
              Item Image
            </label>
            <input
              className='form-field'
